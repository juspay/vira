...
import Vira.State.Type (ForgeInfo (..), Job (..), JobId, JobResult (..), JobStatus (..), OwnerName (..), PRCommit (..), PRState (..), PullRequest (..), prBranchRef)
...
prHandler event = do
  ...
  where
    ...
    handlePROpened = do
      ...
      void $
        async $
          prCheckRunWatcher chan instId owner ghRepo pr.repo pr.prNumber prCommit.commit.id
