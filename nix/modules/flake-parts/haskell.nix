...
in
{
  ...
  perSystem = { self', lib, config, pkgs, ... }:
    let
      # Haskell package set with nix-serve-ng dependencies
      haskellOverlay = hfinal: hprev: {
        base16 = hprev.callHackage "base16" "1.0" { };
        base32 = pkgs.haskell.lib.doJailbreak (hprev.callHackage "base32" "0.4" { });
        charset = hprev.callHackage "charset" "0.3.10" { };
        managed = hprev.callHackage "managed" "1.0.10" { };
      };
      pkgs' = pkgs.extend (final: prev: {
        haskellPackages = prev.haskellPackages.extend haskellOverlay;
      });
    in
    {
